...
class ChatbotConfigRead(BaseModel):
    ...
    # API Key Status (OpenAI / Gemini)
    has_custom_api_key: bool = Field(default=False, description="Whether tenant has a custom API key configured")
    gemini_api_key_preview: Optional[str] = Field(None, description="Masked preview of tenant custom Gemini key")
    openai_api_key_preview: Optional[str] = Field(None, description="Masked preview of tenant custom OpenAI key (e.g. ••••••••X8bQ)")
    system_api_key_configured: bool = Field(default=False, description="Whether global OPENAI_API_KEY or GEMINI_API_KEY is configured in .env")
...
class ChatbotConfigUpdate(BaseModel):
    ...
    gemini_api_key: Optional[str] = Field(None, max_length=255, description="Tenant-specific Gemini API Key.")
    openai_api_key: Optional[str] = Field(None, max_length=255, description="Tenant-specific OpenAI API Key. Send empty string '' to clear and revert to system key.")
...
class OpenAIKeyValidationRequest(BaseModel):
    api_key: Optional[str] = Field(None, description="OpenAI API Key to test")
